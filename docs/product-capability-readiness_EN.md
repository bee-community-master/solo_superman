...
- The default verification suite must include `pnpm verify:core-product-loop`, `pnpm verify:support-bundle`, `pnpm verify:product-capability-readiness`, and `pnpm verify`; supporting commands must include `pnpm verify:codex-live-runtime` without forcing live execution into the default suite, plus the safe `pnpm verify:ready-release -- --plan-only` and `pnpm support:bundle`.
