...
| `research-evidence-loop` | `pnpm verify:research-pipeline` validates mounted `web_search_readonly` provider polling, source-traced result import, evidence matrices/packs, and follow-up question debt; the product capability contract also fixes research follow-up answer-form variety plus the Research tab `Max simultaneous research runs`/`Max research runs per session` limit controls as required behavior |
