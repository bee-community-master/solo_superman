...
## ProductEngine pattern / ProductEngine 패턴
...
## API and local service contract / API와 로컬 서비스 계약

The ProductEngine/application command boundary keeps application commands, route handlers, and reducer decisions separate.

- API routes live in `packages/contracts/src/api/routes.ts` and are checked against `docs/reference.md`.
