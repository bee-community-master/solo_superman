...
Local Codex worker job은 실행 전에 bounded plan으로 기록됩니다. Worker job은 current stage의 markdown/GitHub issue document를 선택하고, generated workspace path, allowed write scope, 필요한 `ImplementationStepLedger` evidence, 금지된 external/credential action, 연결된 `ExecutionAuthorityRecord` ref를 저장합니다. Authority boundary가 없으면 job은 시작하거나 stage를 조용히 advance하지 않고 visible blocked 상태로 남습니다.
