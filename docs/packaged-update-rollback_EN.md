...
This document defines the `solo-superman-packaged-update-rollback.v1` device rollback evidence required before macOS/Windows packaged-artifact updates can become part of a real general release. The current repo/local environment does not contain packaged artifacts or device-lab evidence, so it does not perform real release updates. The repo now includes a credential-free packaged updater runtime; `pnpm verify:packaged-update-rollback` validates the expected macOS/Windows device evidence structure and keeps #267 visibly blocked until that evidence exists. `pnpm verify:packaged-update-rollback:dry-run` uses that runtime to validate fixture update planning, deferral, pre-write failure retry, failed-launch rollback, and preservation boundaries, but it does not replace packaged artifact or device evidence.
...
- Actual device evidence: packaged artifacts, macOS/Windows device or VM runs, and updater rollback logs remain blocked by [#267](https://github.com/bee-community-master/solo_superman/issues/267).
