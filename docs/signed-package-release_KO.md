...
- Release readiness: [`release-readiness_KO.md`](release-readiness_KO.md)와 `pnpm verify:release-readiness`가 signed package evidence를 broad release 필수 gate가 아니라 optional hardening으로 유지합니다.
...
이 dry-run은 실제 signing을 수행하지 않고 macOS/Windows fixture artifact의 checksum, size, signature ref, 공개 certificate metadata와 release manifest signature ref shape를 검증합니다. #293의 local guard이며 #266의 실제 signing/notarization/Authenticode/manifest evidence를 대체하지 않습니다. 실제 evidence run을 `passed`로 바꾸려면 각 run에 구조화된 `evidenceBundle`도 포함해야 합니다.
...
이 모드는 signed artifact를 release claim에 포함하거나 signing hardening evidence가 준비된 PR에서만 통과해야 합니다. 현재 예시 계약은 macOS, Windows, release manifest evidence run이 모두 `blocked`이므로 실패해야 합니다. Release lab이 run을 `passed`로 바꾸면 artifact checksum/size/signature/public certificate metadata, passed checks, redacted evidence refs를 담은 `evidenceBundle`이 필수입니다.
...
- #266을 닫거나 signed artifact를 주장하려면 `pnpm verify:signed-package-preflight -- --require-credentials`, `pnpm verify:signed-package-release -- --require-release-evidence`, 필요 시 `pnpm verify:ready-release -- --include-signed-package`를 통과할 수 있는 redacted release evidence와 구조화된 `evidenceBundle`이 필요합니다.
