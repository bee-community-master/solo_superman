...
- #266을 닫으려면 `pnpm verify:signed-package-preflight -- --require-credentials`, `pnpm verify:signed-package-release -- --require-release-evidence`, `pnpm verify:release-readiness -- --require-ready`를 통과할 수 있는 redacted release evidence와 구조화된 `evidenceBundle`이 필요합니다.
