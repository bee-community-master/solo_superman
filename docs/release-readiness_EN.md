...
- both credential-free and ready-release verification commands are documented.
...
- `pnpm verify` includes the default `pnpm verify:packaged-update-rollback` and `pnpm verify:release-readiness`. PRs can pass credential-free validation while general release remains blocked, but they cannot hide or remove the broad-release blockers.
