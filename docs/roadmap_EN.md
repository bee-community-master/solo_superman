...
Local Codex worker jobs are represented as bounded plans before execution. A worker job selects the current stage's markdown/GitHub issue document, stores the generated workspace path, allowed write scope, required `ImplementationStepLedger` evidence, forbidden external/credential actions, and the linked ready-for-execution `ExecutionAuthorityRecord` file-diff reference scoped to that generated workspace with `no_secret_values`. If that authority boundary is missing or not ready, the job remains visibly blocked instead of starting or silently advancing a stage.
