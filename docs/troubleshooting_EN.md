...
- Confirm the installer runs `nvm install 22` inside WSL, reuses an existing Codex CLI with `command -v codex` and `codex --version` when available, or otherwise runs `npm install -g @openai/codex@latest`, then `codex --version` succeeds without requiring a credential prompt.
