...
| Windows `spawn pnpm ENOENT` during smoke | `pnpm verify:prod-bundle` fails even though `pnpm --version` works in PowerShell, often after a previous install attempt. The same error can appear twice because the installer retries the smoke with alternate ports. | The production smoke runner must spawn `pnpm.cmd` on Windows instead of bare `pnpm`, and it must call `pnpm.cmd -r --if-present build` directly instead of invoking a nested root `pnpm build` script; rerun the updated installer so it pulls the fixed script before smoke verification. |
