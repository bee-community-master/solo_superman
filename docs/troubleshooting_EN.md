...
When collecting external release evidence, run `pnpm release:evidence-bundle -- ./solo-superman-release-evidence-bundle` to generate the full checklist/template, issue-specific Markdown checklists, issue-specific JSON templates, a manifest, and a README in one step. If you only need one issue, run `pnpm release:evidence-checklist -- --format markdown --issue 259` for an issue-ready checklist, `pnpm release:evidence-checklist -- --format comment --issue 267 --output ./issue-267-release-evidence-comment.md` for only the GitHub issue comment body, and `pnpm release:evidence-checklist -- --include-signed-package --format template --issue 266 --output ./issue-266-release-evidence-template.json` for a JSON template that can be filled with redacted evidence refs. The `comment` format fails for mistyped issue numbers that have no evidence items. Validate a filled template with `pnpm verify:release-evidence-template -- --input ./issue-266-release-evidence-template.json` to check placeholders are gone, statuses are `passed`, redaction is confirmed, evidence refs remain secret-free, `evidenceBundleShape` still matches the source contract, pre-gate ready-release commands are recorded without the filled-bundle or aggregate self-command, and `readyReleaseResult.status`/`commandBlockers`/`perCommandBlockers` are filled. Validate a generated or edited bundle with `pnpm verify:release-evidence-bundle -- --bundle-dir ./solo-superman-release-evidence-bundle` to check manifest/on-disk file completeness, ready-release command coverage, and secret-free boundaries; off-manifest scratch notes, logs, or secret-bearing artifacts fail the bundle before it can be shared. After the release lab fills real redacted evidence, add `--require-ready` so every template must be ready/passed. Running `pnpm verify:release-evidence-template`/`pnpm verify:release-evidence-bundle` without input checks fixture templates for default #259/#267 blocker issues and the current bundle contract. Full input templates without `filterIssueNumber` are checked against every source checklist item. The template and bundle verifier do not replace real device evidence or optional signing-lab evidence.

On Windows PowerShell, use `pnpm.cmd verify:prod-bundle`, `pnpm.cmd verify:product-capability-readiness`, `pnpm.cmd verify:release-channel`, `pnpm.cmd verify:windows-real-device`, `pnpm.cmd verify:packaged-update-rollback`, `pnpm.cmd verify:signed-package-preflight`, `pnpm.cmd verify:signed-package-release`, `pnpm.cmd verify:signed-package-release:dry-run`, `pnpm.cmd verify:release-readiness`, `pnpm.cmd verify:release-evidence-template`, `pnpm.cmd verify:release-evidence-bundle`, and `pnpm.cmd verify` so the Node/Corepack command shim runs even when local execution policy blocks `pnpm.ps1`.
...
`pnpm verify:windows-real-device` checks `docs/windows-real-device.example.json` for clean Windows 11 device/VM evidence covering the one-line PowerShell installer, UAC, Node/Git/Corepack/pnpm, WSL/Ubuntu, Codex CLI, Visual C++ runtime, desktop shortcut, rerun safe update, support bundle, and bootstrap/prod-smoke logs tied to #259. The default run passes without real Windows evidence and reports remaining work through `windowsVerificationStatus=blocked` and `blockedDeviceRuns`. In a real release environment, use `pnpm verify:windows-real-device -- --require-device-evidence`; that mode must fail until the Windows device run is passed.
...
`pnpm verify:signed-package-release` checks `docs/signed-package-release.example.json` for macOS codesign/pkgutil/notarization/stapling/Gatekeeper evidence, Windows Authenticode/timestamp evidence, and release manifest checksum/size/signature evidence tied to #266. The default run passes without real signing evidence and reports remaining work through `releaseEvidenceStatus=blocked` and `blockedEvidenceRuns`. `pnpm verify:signed-package-release:dry-run` verifies fixture artifact checksum/size/signature refs and manifest signature-ref shape without credentials, but it does not replace real signing evidence. In a real release environment, use `pnpm verify:signed-package-release -- --require-release-evidence`; that mode must fail until macOS, Windows, and manifest evidence runs are all passed.

`pnpm verify:release-readiness` validates [`release-readiness_EN.md`](release-readiness_EN.md) and [`release-readiness.example.json`](release-readiness.example.json) so packaged updater rollback and Windows real-device gates remain explicitly blocked before general release. Immediately before a real general release, run `pnpm verify:release-readiness -- --require-ready`; the current technical-preview contract must fail that mode until #259 Windows evidence and #267 packaged updater rollback evidence exist. Signed package evidence remains optional hardening for direct/non-store distribution and is included only when signed artifacts are claimed.
...
On Windows PowerShell, use `pnpm.cmd support:bundle`, `pnpm.cmd support:bundle -- --output .\solo-superman-support-bundle.json`, or `pnpm.cmd verify:support-bundle`. The bundle includes OS/Node/pnpm/Codex versions, git branch/head/status, repo remote, package scripts, allowlisted environment values (including `SOLO_CODEX_SDK_LIVE_TURNS`/`SOLO_CODEX_SDK_LIVE_TURN_TIMEOUT_MS`/`SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME` opt-in flags), and credential-free product/release diagnostics summaries for `verify:product-capability-readiness`, `verify:release-channel`, `verify:windows-real-device`, `verify:packaged-update-rollback`, `verify:signed-package-preflight`, `verify:signed-package-release`, `verify:signed-package-release:dry-run`, `verify:release-readiness`, `verify:ready-release -- --plan-only`, `verify:release-evidence-template`, and `verify:release-evidence-bundle` only. When auto implementation live readiness matters, first inspect the recommended `pnpm verify:codex-live-runtime` check. Immediately before final release, use the recommended `pnpm verify:ready-release -- --plan-only` check to inspect the final gate plan, then `pnpm release:evidence-bundle -- <bundle-dir>` to prepare the bundle, validate the filled bundle with `pnpm verify:release-evidence-bundle -- --bundle-dir <bundle-dir> --require-ready`, then run `pnpm verify:ready-release -- --evidence-bundle-dir <bundle-dir>` separately with the release lab bundle. `pnpm verify:support-bundle` validates the support bundle schema, privacy exclusions, script metadata, compact diagnostics, allowlisted env, redaction, and recommended checks. Product/release diagnostics preserve status, blocked capabilities/gates, missing credential group ids, and env names rather than secret values, and the ready-release plan-only summary keeps only plan-safe bundle preparation, planned command list, blocker summary, and issue-specific handoff summary fields. It also keeps `releaseEvidenceIssuePreparation` entries with default #259/#267 evidence item summaries (item id/gate/status plus required check/evidence/unblock counts and compact `evidenceBundle` shape counts), checklist/template/comment paths, template validation commands, and GitHub issue comment commands so the release-lab handoff is visible from the support bundle alone. The generated release evidence bundle itself also carries the same per-issue item counts, count summaries, and compact `evidenceBundle` shape counts in manifest/README `releaseEvidenceIssueSummaries`. Issue checklists/comments then carry the full structured shape field checklists, so labs can continue the handoff without opening the support bundle. It excludes full environment dumps, file contents, browser cookies, OpenAI/GitHub tokens, and ChatGPT web credentials; URL credentials and token/secret/password/API-key shaped values are redacted. Attach the JSON as-is and do not add secrets manually.
...
Live Codex SDK preview turns are disabled by default. Maintainers can opt in with `SOLO_CODEX_SDK_LIVE_TURNS=1` after `codex login status` reports an authenticated local Codex CLI. To prove that the local environment is actually ready for live preview-only turns, run `SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:codex-live-runtime` on macOS/Linux, or `$env:SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:codex-live-runtime` in Windows PowerShell. Set `SOLO_CODEX_SDK_LIVE_TURN_TIMEOUT_MS=<milliseconds>` only when a local live SDK turn needs a longer bounded timeout. The command starts only a temporary local sidecar with a per-run token and temporary app data, reads `/api/v1/runtime/status`, and passes only when the runtime reports `status=available`, `executionMode=live`, `liveTurnExecutionEnabled=true`, and an authenticated Codex account. Running `pnpm verify:codex-live-runtime` without the opt-in flag prints `skipped` evidence instead of pretending live readiness was verified. The app still does not ask for or store API keys, cookies, or ChatGPT web credentials.
...
`pnpm verify:runtime-preview-turn` is the next local-only smoke: by default it uses fixture mode, creates a temporary app-data database, creates a project, queues `/api/v1/runtime/codex/preview`, runs the pending `codex_runtime_preview_effect`, and passes only after a `preview_ready` `ImplementationPlanPreviewArtifact` is visible in runtime activity. To run the same preview-turn path against a live Codex SDK turn, use `SOLO_VERIFY_CODEX_LIVE_PREVIEW_TURN=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:runtime-preview-turn` on macOS/Linux, or `$env:SOLO_VERIFY_CODEX_LIVE_PREVIEW_TURN="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:runtime-preview-turn` in Windows PowerShell. If live execution falls back to manual handoff, the smoke reports `blocked` rather than `passed`. This still verifies preview artifact creation only; it is not a full worker-job end-to-end run.

`pnpm verify:worker-job` is the worker bridge smoke. By default it uses fixture mode, creates temporary app data and a temporary generated-workspace root, creates a planning-ready handoff, creates an auto-implementation workspace run, attaches a ready file-diff `ExecutionAuthorityRecord`, plans and runs the current-stage worker job, imports completed `ImplementationStepLedger` evidence, and advances the run from `initial_pr` to the next review stage. To attempt the same path with live Codex worker turns, use `pnpm verify:worker-job:live` or `SOLO_VERIFY_CODEX_LIVE_WORKER_JOB=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:worker-job` on macOS/Linux, or `$env:SOLO_VERIFY_CODEX_LIVE_WORKER_JOB="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:worker-job` in Windows PowerShell. The package script is a cross-platform shortcut for the same explicit opt-in flags. If live worker execution cannot produce importable ledger evidence, the smoke reports `blocked`. Production deploys, external service mutations, and final-submit actions remain outside this smoke.

`pnpm verify:pr-mutation` is the generated PR mutation smoke. It is always fixture/default and does not call `gh`; instead it creates temporary app data and a temporary generated-workspace root, creates a planning-ready handoff and auto-implementation run with connected fixture remote status, proves approved `open_pr` is blocked before `initial_pr` ledger evidence, applies fixture PR open after that evidence exists, applies fixture `update_pr_body` with current body evidence and generated body markdown including issue document status and missing-test audit summaries, blocks `merge_pr` before `final_verify_pr_update`, blocks merge without current PR body evidence, applies fixture merge with merge readiness evidence, and blocks duplicate merge attempts after the first applied merge. Real GitHub PR writes remain behind explicit approved mutation contracts and are outside default verification.
...
`pnpm verify:auto-implementation-review-loop` is the credential-free review-loop smoke. It creates temporary app data and a local sidecar, creates a planning-ready auto-implementation run, records a fixture applied PR merge mutation before `merge_main`, records post-merge verification evidence, then completes every canonical stage from `initial_pr` through `merge_main` through the existing stage endpoint. Each stage must carry `ImplementationStepLedger` evidence with two no-finding feature/repository code-review passes, two no-finding changed-code/repository clean-code passes, zero-gap missing-test audit evidence, and passing test evidence. Real GitHub writes, deploys, browser actions, public-web polling, and secret access remain outside this scope.
...
`pnpm verify:service-page-pipeline` is the credential-free service page-use pipeline smoke. It creates temporary app data and a local sidecar, serves a loopback mock setup page, verifies service-scoped browser actions are blocked before a matching permission, grants read/preview permission, requires permission/action echo, deletes retained artifact refs to audit-metadata-only, revokes the permission, proves later browser actions are blocked, verifies a per-action fill-draft permission, and keeps final submit blocked without passing production-mutation contract evidence. `pnpm verify:production-mutation-contract` separately verifies the confirmation-card, ExecutionAuthorityRecord, redaction, approval, rollback, audit, and no-secret evidence requirements that would be needed before final-submit readiness. Real service accounts, browser profiles, credential/session/token custody, production deploys, final-submit execution, and external network access remain outside this smoke.

`pnpm verify:research-pipeline` is the credential-free research pipeline smoke. It creates temporary app data and a local sidecar, creates a public-web allowlist, starts a mounted `web_search_readonly` read-only research run, then uses provider polling to import the provider fixture as a source-traced result import. It passes only when the provider-polled run remains `research_insufficient` with an insufficient quality gate, the Research projection exposes an evidence matrix/evidence pack/review card, and the Decision Queue exposes research-generated follow-up question debt. `pnpm verify` runs product capability readiness, the clarification pipeline, clarification volume, research, browser delegation, service page-use, single-session product loop, readiness-to-implementation, and auto-implementation aggregate smokes together; `pnpm verify:core-product-loop` also includes the single-session product loop and readiness-to-implementation handoff so the default local verification covers the question/research/planning-ready/implementation critical paths and staged review protocol without duplicating the review-loop smoke. When public web access is available, `pnpm verify:single-session-product-loop:live-web` additionally runs the same pet-lifecycle path through the live browser-search adapter branch and fails if only fixture URLs are imported. `pnpm verify:single-session-product-loop:worker` extends the same-session fixture path by reusing that Planning Handoff run for a Codex worker job, completed ImplementationStepLedger evidence, and stage advancement beyond `initial_pr`; `pnpm verify:single-session-product-loop:live-implementation` is the explicit opt-in variant that combines live public-web research with live Codex worker execution and reports `blocked` if either live leg cannot produce evidence.
