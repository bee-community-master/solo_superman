...
`pnpm verify:worker-job` is the worker bridge smoke. By default it uses fixture mode, creates temporary app data and a temporary generated-workspace root, creates a planning-ready handoff, creates an auto-implementation workspace run, attaches a ready file-diff `ExecutionAuthorityRecord`, plans and runs the current-stage worker job, imports completed `ImplementationStepLedger` evidence, and advances the run from `initial_pr` to the next review stage. To attempt the same path with live Codex worker turns, use `pnpm verify:worker-job:live` or `SOLO_VERIFY_CODEX_LIVE_WORKER_JOB=1 SOLO_CODEX_APP_SERVER_LIVE_TURNS=1 pnpm verify:worker-job` on macOS/Linux, or `$env:SOLO_VERIFY_CODEX_LIVE_WORKER_JOB="1"; $env:SOLO_CODEX_APP_SERVER_LIVE_TURNS="1"; pnpm.cmd verify:worker-job` in Windows PowerShell. The package script is a cross-platform shortcut for the same explicit opt-in flags. If live worker execution cannot produce importable ledger evidence, the smoke reports `blocked`. Production deploys, external service mutations, and final-submit actions remain outside this smoke.
