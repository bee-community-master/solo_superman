...
- The Research tab can still start currently ready planned public-web research tasks as a bounded batch, respecting the active allowlist and concurrency budget while keeping each run on the existing per-task read-only research path. Maintainers must be able to run a credential-free research pipeline smoke that proves mounted `web_search_readonly` provider polling, source-traced result import, insufficient quality gate review, evidence matrix/pack/review-card synthesis, and Decision Queue follow-up question debt stay connected.
