...
- Auto implementation worker controls must show Codex runtime readiness, execution mode, account status, live-turn flag, manual handoff availability, and ledger-import fallback before an operator tries to run a local worker job; the Implementation runtime panel must let the operator refresh that runtime status without leaving the implementation flow.
