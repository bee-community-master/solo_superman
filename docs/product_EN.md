...
- Long clarification sessions must show question progress with generated, active now, upcoming next, blocked, answered, total follow-up, open follow-up, topic coverage, and remaining follow-up budget counts. Maintainers must be able to run a credential-free clarification pipeline smoke that verifies the question loop from idea intake through initial spec analysis, active question batch, answer submission, follow-up/research debt, and Planning Handoff blocker evidence.
...
- The Research tab can still start currently ready planned public-web research tasks as a bounded batch, respecting the active allowlist and concurrency budget while keeping each run on the existing per-task read-only research path. Maintainers must be able to run a credential-free research pipeline smoke that verifies the read-only research run, source-traced result import, evidence synthesis, Research projection, and Decision Queue follow-up debt stay connected.
...
- ChatGPT local browser delegation must show data disclosure previews, excluded sensitive fields, policy/session-ownership verdicts, approval/authority refs, browser evidence refs, revoke controls, and result-import gates before treating an external AI workspace as safe enough for visible handoff. Maintainers must be able to run a credential-free browser delegation pipeline smoke that proves a loopback browser action, approved ChatGPT delegation run, projection refetch, and revoke path without real ChatGPT credentials or external network access.
- Auto implementation worker controls must show Codex runtime readiness, checked-at evidence, adapter/schema/transport evidence, execution mode, account status, live-turn flag, manual handoff availability, and ledger-import fallback before an operator tries to run a local worker job; the Implementation runtime panel must let the operator refresh that runtime status and see the same readiness evidence, including execution mode, Codex account, live-turn state, and manual handoff state, without leaving the implementation flow. Maintainers must also have opt-in runtime verification commands that distinguish skipped, blocked, and passed readiness evidence instead of making credential-gated live readiness part of the default verification suite, plus credential-free fixture smokes that prove a runtime preview request queues `codex_runtime_preview_effect`, runs the effect executor, persists a `preview_ready` runtime artifact, completes one bounded auto-implementation worker job through ledger import and stage advancement, exercises generated PR open/body-update/merge mutation guards without real GitHub writes, proves every canonical auto-implementation stage can complete with code-review, clean-code, and test evidence in a review-loop smoke, and aggregates the critical-path checks into credential-free pipeline smoke coverage.
