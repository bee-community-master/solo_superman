...
- Codex generated JSON initial questions and deterministic fallback questions must both carry ambiguity-reduction algorithm metadata. Each question tags the weakest goal/scope/constraint/success/context/decision-authority/assumption-pressure dimension, separates fact-checking/current research/human judgment routes, and each generated set must include at least one pressure question plus a source-seeking research task.
...
- Question cards and generated follow-ups must pick the answer form that fits the concrete question intent: open-question subjective/narrative text, binary stance, one-of-many single choice, one-or-more multi-select, ranked, evidence judgment, or experiment-plan answers. The app must not force every answer into a pro/con shape.
...
- Answer submission must not hold the user hostage until background research starts or automatic queue refills finish. Once the answer is persisted, the input flow is released first; public-web research starts/failures and queue refill continue as quiet follow-up work or separate recoverable errors.
...
- Research-generated additional questions must re-enter the Decision Queue as answerable, source-traceable follow-up debt, not remain hidden as evidence-card notes only; when evidence synthesis creates a follow-up question, it must also create a planned research task whose `sourceQueueItemId` points back to that same source trace and queue the corresponding `research_evidence_effect` wait task. Run-status polling that ingests provider evidence must refresh the Queue and Research surfaces that expose that debt, and Planning Handoff build-slice evidence must carry the same research follow-up provenance forward. Those follow-up questions must choose the answer form from the concrete question intent: open_text narrative answers, binary_choice pro/con decisions, single_choice one-of-many choices, multi_select one-or-more selections, ranked_choice, or evidence_judgment. They must not force every answer into a pro/con stance.
...
- The Research tab can still start currently ready planned public-web research tasks as a bounded batch, respecting the active allowlist and concurrency budget while keeping each run on the existing per-task read-only research path. Users must be able to tune Max simultaneous research runs and Max research runs per session from the Research tab, and those limits apply to both manual starts and answer-triggered automatic public-web research starts. Provider-polled research results must be saved as markdown memory so duplicate research can cite existing evidence, but when a user asks for wider follow-up research or evidence synthesis starts a generated follow-up research task, the existing memory is attached as baseline context while a new run still starts. Maintainers must be able to run a credential-free research pipeline smoke that proves mounted `web_search_readonly` provider polling, source-traced result import, insufficient quality gate review, markdown memory persistence/reuse, evidence matrix/pack/review-card synthesis, Decision Queue follow-up question debt, and generated follow-up research runs stay connected. In environments where real public web access is available, `pnpm verify:research-pipeline:live-web` must also be available as an opt-in Playwright-backed check that imports real public source URLs instead of fixture URLs. That live-web check must not log in, bypass CAPTCHA, or use paid-service access; it should surface blocked/failure evidence directly.
