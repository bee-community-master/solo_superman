...
Auto implementation stage는 timer만으로 completed가 될 수 없습니다. Runner는 각 tick을 기록해야 하며, completed `ImplementationStepLedger` step이 commit/no-code evidence, code-review 및 clean-code review streak, passing tests, blocker history를 증명할 때만 stage를 complete할 수 있습니다. Live Codex work는 local/sandboxed 상태를 유지하고, production 또는 external final-submit action은 future contract가 명시적으로 열기 전까지 blocked입니다.
