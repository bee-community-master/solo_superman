...
Auto implementation stages cannot be completed from timers alone. The runner must record each tick and may only complete a stage when a completed `ImplementationStepLedger` step proves commit/no-code evidence, code-review and clean-code review streaks, passing tests, and blocker history. Live Codex work stays local/sandboxed, and production or external final-submit actions remain blocked until a future contract explicitly opens them.
