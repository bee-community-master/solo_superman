...
Auto implementation stages cannot be completed from timers alone. The runner must record each tick and may only complete a stage when a completed `ImplementationStepLedger` step proves commit/no-code evidence, code-review and clean-code review streaks, passing tests, and blocker history. Local Codex worker jobs must first store a bounded plan with the current issue document, allowed workspace write scope, required evidence, forbidden external/credential actions, and a ready-for-execution file-diff `ExecutionAuthorityRecord` reference scoped to that generated workspace with `no_secret_values`; missing or non-ready authority keeps the job visibly blocked. Worker completion also requires a validated completed `ImplementationStepLedger` step and does not bypass the existing stage endpoint. Live Codex work stays local/sandboxed, and production or external final-submit actions remain blocked until a future contract explicitly opens them.
