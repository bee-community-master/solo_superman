...
- Codex generated JSON 초기 질문과 deterministic fallback 질문은 모두 ambiguity-reduction algorithm metadata를 가져야 합니다. 각 질문은 목표·범위·제약·성공 기준·맥락·결정권·assumption pressure 중 가장 약한 차원을 표시하고, fact-checking/current research/human judgment route를 분리해야 하며, 질문 묶음에는 최소 하나의 pressure question과 source-seeking research task가 포함되어야 합니다. Fallback research target도 아이디어 도메인에 맞는 공개 source area, 현재 가정을 약하게 만드는 반례, 사용자가 여전히 판단해야 할 내용을 함께 남겨야 합니다.
