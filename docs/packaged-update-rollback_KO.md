...
이 문서는 macOS/Windows packaged artifact 기반 자동 업데이트를 실제 general release로 열기 전에 필요한 `solo-superman-packaged-update-rollback.v1` device rollback evidence 계약입니다. 현재 repo/local 환경에는 packaged artifact와 device lab evidence가 없으므로 실제 release update는 수행하지 않습니다. 대신 repo에는 credential-free packaged updater runtime이 있으며, `pnpm verify:packaged-update-rollback`이 macOS/Windows device evidence 구조와 #267의 blocked 상태를 검증합니다. `pnpm verify:packaged-update-rollback:dry-run`은 그 runtime으로 fixture 기반 update plan, defer, pre-write failure retry, failed-launch rollback, preservation 경계를 검증하지만 실제 packaged artifact나 device evidence를 대체하지 않습니다.
...
- Actual device evidence: packaged artifact, macOS/Windows device/VM, updater rollback logs가 준비될 때까지 [#267](https://github.com/bee-community-master/solo_superman/issues/267)로 blocked입니다.
