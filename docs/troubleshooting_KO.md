...
`pnpm verify:research-pipeline`은 credential-free research pipeline smoke입니다. 임시 app data와 local sidecar를 만들고 public-web allowlist를 생성한 뒤 mounted `web_search_readonly` read-only research run을 시작하며, provider polling 결과를 source-traced result import로 가져옵니다. 통과하려면 provider-polled run이 insufficient quality gate와 함께 `research_insufficient`로 남고, Research projection에 evidence matrix/evidence pack/review card가 보이며, Decision Queue에 research-generated follow-up question debt가 노출되어야 합니다. `pnpm verify`는 product capability readiness, clarification pipeline, clarification volume, research, browser delegation, service page-use, readiness-to-implementation, auto-implementation aggregate smoke를 함께 실행하며, `pnpm verify:core-product-loop`도 readiness-to-implementation handoff를 포함하므로 기본 로컬 검증에서 질문/리서치/planning-ready/구현 critical path와 staged review protocol을 모두 확인합니다.
