...
외부 release evidence를 모을 때는 `pnpm release:evidence-checklist -- --format markdown --issue 259`로 이슈용 checklist를 만들고, `pnpm release:evidence-checklist -- --format template --issue 266 --output ./issue-266-release-evidence-template.json`으로 redacted evidence ref를 채울 JSON template을 생성합니다. Template은 `pending` placeholder만 만들며 실제 device/signing lab evidence를 대체하지 않습니다.

Windows PowerShell에서는 local execution policy가 `pnpm.ps1`을 막아도 Node/Corepack command shim이 실행되도록 `pnpm.cmd verify:prod-bundle`, `pnpm.cmd verify:product-capability-readiness`, `pnpm.cmd verify:release-channel`, `pnpm.cmd verify:windows-real-device`, `pnpm.cmd verify:packaged-update-rollback`, `pnpm.cmd verify:signed-package-preflight`, `pnpm.cmd verify:signed-package-release`, `pnpm.cmd verify:signed-package-release:dry-run`, `pnpm.cmd verify:release-readiness`, `pnpm.cmd verify`를 사용합니다.
