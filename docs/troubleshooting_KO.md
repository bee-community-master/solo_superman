...
외부 release evidence를 모을 때는 `pnpm release:evidence-checklist -- --format markdown --issue 259`로 이슈용 checklist를 만들고, `pnpm release:evidence-checklist -- --format template --issue 266 --output ./issue-266-release-evidence-template.json`으로 redacted evidence ref를 채울 JSON template을 생성합니다. 채운 template은 `pnpm verify:release-evidence-template -- --input ./issue-266-release-evidence-template.json`으로 placeholder 제거, `passed` 상태, redaction 확인, secret-free evidence ref를 검사합니다. Template은 실제 device/signing lab evidence를 대체하지 않습니다.
