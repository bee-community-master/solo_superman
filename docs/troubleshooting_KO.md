...
외부 release evidence를 모을 때는 `pnpm release:evidence-bundle -- ./solo-superman-release-evidence-bundle`로 full checklist/template, issue별 Markdown checklist, issue별 JSON template, manifest, README를 한 번에 만들 수 있습니다. 단일 이슈만 필요하면 `pnpm release:evidence-checklist -- --format markdown --issue 259`로 이슈용 checklist를 만들고, `pnpm release:evidence-checklist -- --format template --issue 266 --output ./issue-266-release-evidence-template.json`으로 redacted evidence ref를 채울 JSON template을 생성합니다. 채운 template은 `pnpm verify:release-evidence-template -- --input ./issue-266-release-evidence-template.json`으로 placeholder 제거, `passed` 상태, redaction 확인, secret-free evidence ref, 모든 ready-release command 실행 기록을 검사합니다. 입력 없이 실행한 `pnpm verify:release-evidence-template`은 #259/#266/#267 blocker issue fixture template을 모두 검사합니다. `filterIssueNumber`가 없는 전체 input template은 source checklist 전체 item과 대조됩니다. Template은 실제 device/signing lab evidence를 대체하지 않습니다.
