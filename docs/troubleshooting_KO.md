...
외부 release evidence를 모을 때는 `pnpm release:evidence-bundle -- ./solo-superman-release-evidence-bundle`로 full checklist/template, issue별 Markdown checklist, issue별 JSON template, manifest, README를 한 번에 만들 수 있습니다. 단일 이슈만 필요하면 `pnpm release:evidence-checklist -- --format markdown --issue 259`로 이슈용 checklist를 만들고, `pnpm release:evidence-checklist -- --format comment --issue 267 --output ./issue-267-release-evidence-comment.md`로 GitHub issue comment 본문만 만들 수 있으며, `comment` 형식은 evidence item이 없는 잘못된 이슈 번호에서 실패합니다. `pnpm release:evidence-checklist -- --include-signed-package --format template --issue 266 --output ./issue-266-release-evidence-template.json`으로 redacted evidence ref를 채울 JSON template을 생성합니다. 채운 template은 `pnpm verify:release-evidence-template -- --input ./issue-266-release-evidence-template.json`으로 placeholder 제거, `passed` 상태, redaction 확인, secret-free evidence ref, source contract와 맞는 `evidenceBundleShape`, filled-bundle/aggregate self-command을 제외한 pre-gate ready-release command 실행 기록과 `readyReleaseResult.status`/`commandBlockers`/`perCommandBlockers`를 검사합니다. 생성/수정한 bundle은 `pnpm verify:release-evidence-bundle -- --bundle-dir ./solo-superman-release-evidence-bundle`로 manifest와 실제 디스크 파일의 completeness/listing, ready-release command 포함 여부, secret-free 경계를 검사하고, manifest에 없는 scratch note, log, secret-bearing artifact가 섞이면 공유 전 실패합니다. 실제 redacted evidence를 모두 채운 release lab bundle은 `--require-ready`로 모든 template의 ready/passed 상태까지 검사합니다. 입력 없이 실행한 `pnpm verify:release-evidence-template`/`pnpm verify:release-evidence-bundle`은 기본 #259/#267 blocker issue fixture template과 현재 bundle 계약을 모두 검사합니다. #266 signed-package hardening template은 `--include-signed-package`를 명시해 생성합니다. `filterIssueNumber`가 없는 전체 input template은 source checklist 전체 item과 대조됩니다. Template과 bundle verifier는 실제 device evidence 및 optional signing lab evidence를 대체하지 않습니다.

Windows PowerShell에서는 local execution policy가 `pnpm.ps1`을 막아도 Node/Corepack command shim이 실행되도록 `pnpm.cmd verify:prod-bundle`, `pnpm.cmd verify:product-capability-readiness`, `pnpm.cmd verify:release-channel`, `pnpm.cmd verify:windows-real-device`, `pnpm.cmd verify:packaged-update-rollback`, `pnpm.cmd verify:signed-package-preflight`, `pnpm.cmd verify:signed-package-release`, `pnpm.cmd verify:signed-package-release:dry-run`, `pnpm.cmd verify:release-readiness`, `pnpm.cmd verify:release-evidence-template`, `pnpm.cmd verify:release-evidence-bundle`, `pnpm.cmd verify`를 사용합니다.
...
Windows PowerShell에서는 `pnpm.cmd support:bundle`, `pnpm.cmd support:bundle -- --output .\solo-superman-support-bundle.json`, `pnpm.cmd verify:support-bundle`을 사용합니다. Bundle은 OS/Node/pnpm/Codex version, git branch/head/status, repo remote, package script, allowlisted environment(`SOLO_CODEX_SDK_LIVE_TURNS`/`SOLO_CODEX_SDK_LIVE_TURN_TIMEOUT_MS`/`SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME` opt-in flag 포함), 그리고 `verify:product-capability-readiness`/`verify:release-channel`/`verify:windows-real-device`/`verify:packaged-update-rollback`/`verify:signed-package-preflight`/`verify:signed-package-release`/`verify:signed-package-release:dry-run`/`verify:release-readiness`/`verify:ready-release -- --plan-only`/`verify:release-evidence-template`/`verify:release-evidence-bundle`의 credential-free product/release diagnostics summary만 포함합니다. Auto implementation live readiness가 필요하면 recommended checks의 `pnpm verify:codex-live-runtime`을 먼저 확인합니다. Final release 직전에는 recommended checks의 `pnpm verify:ready-release -- --plan-only`로 final gate plan을 확인하고 `pnpm release:evidence-bundle -- <bundle-dir>`로 bundle을 만들고 filled bundle을 `pnpm verify:release-evidence-bundle -- --bundle-dir <bundle-dir> --require-ready`로 검증한 뒤 `pnpm verify:ready-release -- --evidence-bundle-dir <bundle-dir>`를 release lab bundle과 함께 별도로 실행합니다. `pnpm verify:support-bundle`은 이 support bundle의 schema, privacy exclusion, script metadata, compact diagnostics, allowlisted env, redaction, recommended check를 검증합니다. Product/release diagnostics는 status, blocked capability/gate, missing credential group id와 env name처럼 값이 아닌 이름만 보존하고, ready-release plan-only summary에는 plan-safe bundle preparation command, planned command list, blocker summary, issue-specific handoff summary만 담습니다. 또한 `releaseEvidenceIssuePreparation`에 기본 #259/#267별 evidence item 요약(item id/gate/status와 필수 check/evidence/unblock count, compact `evidenceBundle` shape count), checklist/template/comment 경로, template 검증 명령, GitHub issue comment 명령을 함께 담아 release lab handoff가 support bundle만으로도 보이게 합니다. 생성된 release evidence bundle 자체도 manifest/README의 `releaseEvidenceIssueSummaries`에 같은 issue별 item count, count 요약, compact `evidenceBundle` shape count를 담습니다. 이어서 issue checklist/comment에는 전체 structured shape field checklist를 포함하므로 support bundle 없이도 lab handoff를 이어받을 수 있습니다. Full environment dump, file contents, browser cookies, OpenAI/GitHub token, ChatGPT web credential은 수집하지 않으며 URL credential, token/secret/password/API-key shaped value는 redacted로 남깁니다. 실패 리포트에 이 JSON을 첨부하되, 수동으로 secret을 추가하지 마세요.
...
Live Codex SDK preview turn은 기본 비활성입니다. Maintainer는 `codex login status`가 local Codex CLI 인증 상태를 보고한 뒤 `SOLO_CODEX_SDK_LIVE_TURNS=1`로 opt-in할 수 있습니다. 로컬 환경이 실제 live preview-only turn 준비 상태인지 증명하려면 macOS/Linux에서는 `SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:codex-live-runtime`, Windows PowerShell에서는 `$env:SOLO_VERIFY_CODEX_SDK_LIVE_RUNTIME="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:codex-live-runtime`을 실행합니다. local live SDK turn에 더 긴 bounded timeout이 필요할 때만 `SOLO_CODEX_SDK_LIVE_TURN_TIMEOUT_MS=<milliseconds>`를 설정합니다. 이 명령은 per-run token과 임시 app data로 local sidecar만 띄운 뒤 `/api/v1/runtime/status`를 읽고, runtime이 `status=available`, `executionMode=live`, `liveTurnExecutionEnabled=true`, authenticated Codex account를 모두 보고할 때만 통과합니다. Opt-in 없이 `pnpm verify:codex-live-runtime`만 실행하면 live readiness를 검증한 것처럼 보이지 않도록 `skipped` evidence를 출력합니다. App은 이 경우에도 API key, cookie, ChatGPT web credential을 요청하거나 저장하지 않습니다.
...
`pnpm verify:runtime-preview-turn`은 그 다음 local-only smoke입니다. 기본값은 fixture mode이며 임시 app-data database를 만들고, project를 생성한 뒤 `/api/v1/runtime/codex/preview`를 queue하고, pending `codex_runtime_preview_effect`를 실행해 runtime activity에 `preview_ready` `ImplementationPlanPreviewArtifact`가 보일 때만 통과합니다. 같은 preview-turn 경로를 live Codex SDK turn으로 검증하려면 macOS/Linux에서는 `SOLO_VERIFY_CODEX_LIVE_PREVIEW_TURN=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:runtime-preview-turn`, Windows PowerShell에서는 `$env:SOLO_VERIFY_CODEX_LIVE_PREVIEW_TURN="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:runtime-preview-turn`을 실행합니다. Live 실행이 manual handoff로 fallback되면 이 smoke는 `passed`가 아니라 `blocked`를 보고합니다. 이 명령은 preview artifact 생성까지만 검증하며 full worker-job end-to-end run은 아직 아닙니다.

`pnpm verify:worker-job`은 worker bridge smoke입니다. 기본값은 fixture mode이며 임시 app data와 임시 generated-workspace root를 만들고, planning-ready handoff를 생성한 뒤 auto-implementation workspace run을 만들며, ready file-diff `ExecutionAuthorityRecord`를 붙이고 current-stage worker job을 plan/run하여 completed `ImplementationStepLedger` evidence를 import하고 run을 `initial_pr`에서 다음 review stage로 advance할 때만 통과합니다. 같은 경로를 live Codex worker turn으로 시도하려면 macOS/Linux에서는 `pnpm verify:worker-job:live` 또는 `SOLO_VERIFY_CODEX_LIVE_WORKER_JOB=1 SOLO_CODEX_SDK_LIVE_TURNS=1 pnpm verify:worker-job`, Windows PowerShell에서는 `$env:SOLO_VERIFY_CODEX_LIVE_WORKER_JOB="1"; $env:SOLO_CODEX_SDK_LIVE_TURNS="1"; pnpm.cmd verify:worker-job`을 실행합니다. Package script는 같은 명시적 opt-in flag를 켜는 cross-platform shortcut입니다. Live worker execution이 import 가능한 ledger evidence를 만들지 못하면 이 smoke는 `blocked`를 보고합니다. Production deploy, external service mutation, final-submit action은 이 smoke 범위 밖에 남아 있습니다.

`pnpm verify:pr-mutation`은 generated PR mutation smoke입니다. 기본값은 항상 fixture이며 `gh`를 호출하지 않습니다. 대신 임시 app data와 임시 generated-workspace root를 만들고, planning-ready handoff와 connected fixture remote status의 auto-implementation run을 만든 뒤, approved `open_pr`이 `initial_pr` ledger evidence 전에는 막히는지, 해당 evidence 이후 fixture PR open이 적용되는지, current body evidence와 issue document status 및 missing-test audit summary를 포함한 generated body markdown이 있는 fixture `update_pr_body`가 적용되는지, `final_verify_pr_update` 전에는 `merge_pr`이 막히는지, current PR body evidence가 없으면 merge가 막히는지, merge readiness evidence가 있으면 fixture merge가 적용되는지, 첫 merge 이후 duplicate merge가 막히는지를 검증합니다. 실제 GitHub PR write는 계속 explicit approved mutation contract 뒤에 있으며 default verification 범위 밖입니다.
...
`pnpm verify:service-page-pipeline`은 credential-free service page-use pipeline smoke입니다. 임시 app data와 local sidecar를 만들고 loopback mock setup page를 띄운 뒤, matching permission 전 service-scoped browser action 차단, read/preview permission grant, permission/action echo 요구, retained artifact ref의 audit-metadata-only 삭제, revoke 이후 browser action 차단, per-action fill-draft permission, 통과한 production-mutation contract evidence가 없는 final submit 차단을 검증합니다. `pnpm verify:production-mutation-contract`는 final-submit readiness에 필요한 confirmation-card, ExecutionAuthorityRecord, redaction, approval, rollback, audit, no-secret evidence 요구사항을 별도로 검증합니다. 실제 service account, browser profile, credential/session/token custody, production deploy, final-submit execution, 외부 네트워크 접근은 이 smoke 범위 밖입니다.

`pnpm verify:research-pipeline`은 credential-free research pipeline smoke입니다. 임시 app data와 local sidecar를 만들고 public-web allowlist를 생성한 뒤 mounted `web_search_readonly` read-only research run을 시작하며, provider polling 결과를 source-traced result import로 가져옵니다. 통과하려면 provider-polled run이 insufficient quality gate와 함께 `research_insufficient`로 남고, Research projection에 evidence matrix/evidence pack/review card가 보이며, Decision Queue에 research-generated follow-up question debt가 노출되어야 합니다. `pnpm verify`는 product capability readiness, clarification pipeline, clarification volume, research, browser delegation, service page-use, single-session product loop, readiness-to-implementation, auto-implementation aggregate smoke를 함께 실행하며, `pnpm verify:core-product-loop`도 single-session product loop와 readiness-to-implementation handoff를 포함하므로 기본 로컬 검증에서 질문/리서치/planning-ready/구현 critical path와 staged review protocol을 모두 확인합니다. public web 접근이 가능할 때는 `pnpm verify:single-session-product-loop:live-web`로 같은 반려동물 전생애주기 경로가 live browser-search adapter branch에서 fixture가 아닌 source URL을 import하는지도 추가 검증할 수 있습니다. `pnpm verify:single-session-product-loop:worker`는 같은 세션의 fixture 경로를 확장해 그 Planning Handoff run을 그대로 재사용하여 Codex worker job, completed ImplementationStepLedger evidence, `initial_pr` 이후 stage advancement까지 검증합니다. `pnpm verify:single-session-product-loop:live-implementation`은 live public-web research와 live Codex worker execution을 한 same-session proof에서 함께 실행하는 명시적 opt-in 경로이며, 둘 중 하나라도 evidence를 만들지 못하면 `blocked`를 보고합니다.
