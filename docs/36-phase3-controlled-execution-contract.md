...
| 2. `shell_command` | Run only non-destructive allowlisted commands in a bounded command sandbox. | Slice 1 green; command allowlist, workspace/cwd boundary, max duration, and compensating-action rollback are present. | Default allowlist is repo `package.json` scripts plus limited read-only diagnostics such as `ls`, `cat`, `rg`, and `git status`; `rg` must name explicit non-sensitive file paths, so implicit or recursive workspace scans are blocked. Raw shell mutation outside that allowlist is blocked. Read-only diagnostics time out at 30 seconds; test/typecheck/lint/docs verify commands time out at 10 minutes; build/full verify commands time out at 20 minutes; dev server commands require a separate preview mode with automatic shutdown/kill evidence. Exit code, duration, redacted stdout/stderr summary, and compensating-action/rollback refs are captured; destructive shell commands, deploy, force reset/delete, system setting mutation, credential value access, unsafe diagnostic flags, and symlink path escape return `blocked`. |
...
### Slice 2 current code-backed surface

The `shell_command` slice is represented in code by:

- `ExecuteShellCommandRequest`, `ShellCommandRunSummaryDto`, and `ShellCommandExecutionResult` in `packages/contracts/src/projections/execution-authority.ts`.
- `POST /api/v1/execution-authorities/:authorityRecordId/shell-command` in the sidecar route catalog and Hono router.
- `apps/sidecar/src/product-engine/shell-command-adapter.ts`, which hashes the exact argv-style command preview, validates the authority record, checks the approved workspace/cwd and command allowlist, blocks credential-bearing paths/arguments, unsafe diagnostic flags, symlink path escape, and destructive/deploy/system commands, runs without shell interpolation, enforces class-specific timeouts, and returns exit code, duration, and redacted stdout/stderr summaries.
- `executionAuthorityRepository.updateExecutionOutcome`, which persists `completed`, `blocked`, `failed`, or `partial` terminal state plus evidence/audit refs onto the same `ExecutionAuthorityRecord` so the user-visible ledger does not hide failed/blocked attempts.
