...
6. **Source-driven synthesis/UI trigger PR (#74)**: replace generic single task/PR/build scaffold output with source-driven product-context, evidence/queue, and readiness/residual-risk tasks. Add a Web `Run Planning Handoff gate` control that posts current projection source refs to `CreatePlanningHandoff` and then displays the returned final/blocker projection without adding execution authority.
...
- The Web gate trigger is allowed to create local final/blocker Planning Handoff metadata only. It must not introduce file patch, shell command, browser action, deploy, credential, external mutation, or active delegation controls.
