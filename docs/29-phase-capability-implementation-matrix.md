...
이 문서는 PR/issue 실행 순서, 세부 schema, DTO field, API endpoint, package layout을 확정하지 않는다. 그런 결정은 Phase별 구현계획 문서나 후속 GitHub issue에서 다룬다.
...
| --- | --- | --- | --- | --- | --- | --- |
| Phase 0: 기획 문서 완성 | 사용자는 아직 제품을 쓰는 단계가 아니라, 구현자가 제품 기준을 재결정하지 않아도 되는 상태 | 제품/UX/도메인/아키텍처/보안/런타임/저장소/API/DTO/운영 문서 계약을 완성하고 dry-run으로 설명 가능한 기준선을 만든다 | 초기 제품 방향과 Phase 1 MVP 범위가 문서화되어 있음 | 문서 간 상충 없음, Phase 1 구현자가 topology/package layout/DB/API/Codex boundary/PR sequence를 다시 결정하지 않아도 됨 | runtime feature 구현으로 확장하지 않음, 문서가 코드보다 앞서는 임시 가정이면 후속 구현 PR에서 ADR-style note로 정리 | `00`~`30`, README, `12-validation-and-dry-run.md`, `22-phase1-implementation-sequence.md` |
| Phase 1: Research 포함 Spec 폐루프 MVP | `Spec-ready`: 막연한 아이디어가 근거, 반대근거, Known Risks, Founder Brief가 있는 Living Product Spec v1로 닫힘 | macOS desktop app, Tauri + React/Vite shell, Node/Hono sidecar, local embedded libSQL + Drizzle, ProductEngine reducer/effect queue, Decision Queue, manual research import, EvidenceMatrix, Codex sandbox preview, completeness score, Founder Brief export | Phase 0 문서 계약과 PR-01~PR-09 순서가 고정됨 | 샘플 아이디어가 project creation -> question batch -> answer -> research_needed -> manual evidence import -> decision approval -> SpecVersion -> completeness -> Founder Brief draft/export까지 dry-run 됨 | 팀 협업, 모바일 앱, 결제/과금, remote sync beyond placeholder, ChatGPT web automation, actual file/shell/browser execution | GitHub issue #2~#11 closed 기준선, `22-phase1-implementation-sequence.md`, `12-validation-and-dry-run.md`, `27-operations-observability-contract.md` |
...
| Phase 2: Execution Planning Handoff | `Planning-ready`: Spec과 해결된 리서치 큐를 PR/issue/task 단위 실행계획으로 바꿀 준비가 됨 | Living Product Spec, terminal Research-updated Queue, Known Risks, prerequisites를 구현 가능한 task breakdown, PR/issue 계획, implementation readiness checklist, unresolved risk/prerequisite 표시로 변환한다 | Phase 1.5A-2 high-impact queue card가 terminal outcome을 가짐. 남은 low/medium risk는 Known Risks/Open Questions/prerequisite로 명시 가능함 | Planning artifact가 unresolved high-impact risk를 숨기지 않고, blocker/prerequisite/assumption/validation dependency를 명시함 | 자동 적용, shell 실행, file patch 실행, browser action 실행을 하지 않음. Provisional plan을 final implementation plan처럼 보여주지 않음 | `03-living-product-spec.md`, `04-decision-queue.md`, `12-validation-and-dry-run.md`, `28-founder-os-product-doctrine.md` |
| Phase 2.5: Browser Automation Preview | `브라우저 자동화 검토 중`: 시장/경쟁/검증 리서치에서 브라우저 action이 필요할 때 preview와 통제권을 먼저 봄 | PlaywrightRuntime, BrowserUseRuntime, ChatGPT Pro web research automation 같은 후보를 action preview, source capture, audit log, revoke control, fallback chain 중심으로 검토한다 | Phase 1 manual prompt handoff가 실제 research bottleneck으로 확인됨. 정책/약관/세션/사용량 제한 리스크 검토, fallback chain, audit/revoke 설계가 있음 | Browser action plan과 source capture가 preview-only artifact로 남고 사용자는 실행 전 action 범위와 revoke 경계를 이해함 | 사용자 승인 없는 form submission, login/payment/sensitive action 자동 실행, ChatGPT 계정 공유 또는 인증정보 대리 보관 금지 | `17-ai-runtime-access-strategy.md`, `10-security-privacy-and-approval.md`, `28-founder-os-product-doctrine.md` |
| Phase 3: Safe Execution Adapter (Controlled Execution) | `안전 실행 대기`: 승인, sandbox, rollback 경계 안에서 실행계획을 적용할 준비가 됨 | File diff preview, shell command preview, browser action preview를 approval-first adapter로 제공하고 sandbox, rollback, audit log, terminal evidence를 연결한다 | Phase 2 task breakdown 품질이 충분히 검증됨. Approval model, sandbox boundary, rollback reference가 안정됨 | 사용자가 승인한 controlled execution만 적용되고, 실행 전 preview와 실행 후 evidence/audit/rollback reference가 남음 | 무승인 실행, credential/destructive/external-production action 자동 실행, blanket delegation, 숨겨진 mutation 금지 | `10-security-privacy-and-approval.md`, `17-ai-runtime-access-strategy.md`, `24-codex-prompt-output-contract.md`, `28-founder-os-product-doctrine.md` |
| Phase 4: Optional Cloud and Mobile Monitor | 데스크톱 밖에서도 질문/승인/진행 상태를 놓치지 않음 | Opt-in project sync, mobile approval monitor, push notification, task progress monitor, remote approval queue visibility를 local-first 원칙을 깨지 않는 보조 capability로 제공한다 | Desktop session 밖에서도 질문/승인을 처리하려는 반복 니즈가 확인됨. Phase 3 approval/audit model이 안정됨 | Sync/monitor는 desktop/local source of truth와 충돌하지 않고, opt-in/revoke/recovery 경계가 설명 가능함 | Mobile-first rewrite, mandatory cloud account, billing, team collaboration, cloud를 기본 source of truth로 만드는 결정 금지 | `10-security-privacy-and-approval.md`, `11-roadmap-and-phase-boundaries.md`, `28-founder-os-product-doctrine.md` |
...
- Phase 2는 unresolved high-impact Research-updated Queue card가 있으면 final handoff로 닫을 수 없다.
- Phase 3는 Phase 2 planning artifact의 approval, sandbox, rollback requirement가 불충분하면 열 수 없다.
...
- [ ] Phase 2 gate가 high-impact Research-updated Queue terminal outcome을 요구한다.
- [ ] Phase 1.5B와 Phase 2.5는 preview/hint-only이며 실제 실행을 허용하지 않는다.
- [ ] Phase 3 전에는 file/shell/browser/deploy/external mutation이 제품 capability로 구현되지 않는다.
