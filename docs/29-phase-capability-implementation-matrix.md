...
이 문서는 PR/issue 실행 순서, 세부 schema, DTO field, API endpoint, package layout을 확정하지 않는다. 그런 결정은 Phase별 구현계획 문서나 후속 GitHub issue에서 다룬다. 예외적으로 Phase 2 Planning Handoff artifact/report schema의 canonical docs-level 계약은 `31-phase2-planning-handoff-contract.md`가 소유하고, Phase 2 DTO/API/storage/gate exact implementation defaults는 `32-phase2-implementation-preflight-contract.md`가 소유한다.
