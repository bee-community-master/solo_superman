...
| `research-evidence-loop` | `pnpm verify:research-pipeline`이 mounted `web_search_readonly` provider polling, source-traced result import, evidence matrix/pack, follow-up question debt를 검증하고, product capability contract가 research follow-up의 answer-form variety와 Research tab의 `Max simultaneous research runs`/`Max research runs per session` 한도 조절 UX를 필수 행동으로 고정 |
