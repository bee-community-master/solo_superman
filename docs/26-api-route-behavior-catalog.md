...
| `GET /api/v1/projects/:projectId/research-runs/:researchRunId/status` | Auth required. Params: `projectId`, `researchRunId`. No body. | `commandType: none`; project-scoped ResearchRun status query. | `ApiSuccessEnvelope<ResearchRunStatusDto>` with selected run, stable `statusUrl`, collection `refetchUrl`, and projection hints. | No effect. Polling this URL recovers missed SSE/status updates. | `RESOURCE_NOT_FOUND` if project or run missing/mismatched. |
