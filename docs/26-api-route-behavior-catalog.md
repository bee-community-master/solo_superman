...
| `POST /api/v1/sessions/:sessionId/service-page-use-permissions/:permissionId/revoke` | Auth required. Param/body `sessionId` and `permissionId` must match. Body DTO: `RevokeServicePageUsePermissionRequest` with `idempotencyKey`, `expectedStateVersion`, reason, optional audit refs. | `commandType: RevokeServicePageUsePermission`; deterministic revoke of the latest granted/final-submit-requested service page-use permission. | `accepted_with_projection` with `ServicePageUsePermissionProjection` and latest permission `currentStatus=revoked`. | Persists `ServicePagePermissionRevoked`; no browser adapter call is made. Further browser-action replay also re-checks the revoked permission before returning prior completed evidence. Retained artifact refs remain under a separate durable artifact delete route while audit metadata stays hash/ref/provenance-only. | `RESOURCE_NOT_FOUND` for missing session/projection/latest permission mismatch, `COMMAND_PRECONDITION_FAILED` for already blocked/revoked permissions, `STATE_VERSION_CONFLICT` for stale expected state, `VALIDATION_FAILED` for malformed body, secret-shaped values, or ref-shaped credential/session/token/secret custody values or credential-bearing URL userinfo. |
| `POST /api/v1/sessions/:sessionId/service-page-use-permissions/:permissionId/artifacts/delete` | Auth required. Param/body `sessionId` and `permissionId` must match. Body DTO: `DeleteServicePageUsePermissionArtifactsRequest` with `idempotencyKey`, `expectedStateVersion`, reason, optional audit refs. | `commandType: DeleteServicePageUsePermissionArtifacts`; deterministic deletion of retained prompt/redaction/screenshot/log artifact refs on the addressed permission record within the service page-use projection. | `accepted_with_projection` with `ServicePageUsePermissionProjection`; addressed permission keeps its status/revoke state but `artifactRetention.promptResultScreenshotLogRetention=deleted_audit_metadata_only`, `promptPreviewRef=null`, `redactionPreviewRef=null`, and screenshot/log refs are empty; `latestPermission` remains the newest permission record. | Persists `ServicePageArtifactsDeleted`; no browser adapter call is made. Audit/evidence metadata records deletion while prompt/result/screenshot/log refs are durably removed from the projection. | `RESOURCE_NOT_FOUND` for missing session/projection/addressed permission mismatch, `COMMAND_PRECONDITION_FAILED` when artifact refs were already deleted, `STATE_VERSION_CONFLICT` for stale expected state, `VALIDATION_FAILED` for malformed body, secret-shaped values, or ref-shaped credential/session/token/secret custody values or credential-bearing URL userinfo. |
| `POST /api/v1/sessions/:sessionId/implementation-step-ledger` | Auth required. Param/body `sessionId` must match. Body DTO: `RecordImplementationStepLedgerRequest` with `idempotencyKey`, `expectedStateVersion`, `TrackerDoc`, `ImplementationStepDoc`, target status, optional evidence refs, implementation evidence (`StepCommitRecord` for tracked code/docs/config, or clean `NoCodeStepEvidence` for verification/no-op), separate `CodeReviewRecord` and `CleanCodeReviewRecord`, optional `TestEvidenceRecord` with `verifiedCommitSha`, and optional blocker. | `commandType: RecordImplementationStepLedger`; deterministic ProductEngine ledger command for Codex-built implementation progress. | `accepted_with_projection` with `ImplementationStepLedgerProjection`; completed steps expose commit SHA/no-code baseline, previous commit, diff range, rollback ref, separate reviews, passing tests bound to the same commit/baseline, and progress report. Missing or failed evidence returns a `blocked` projection rather than false completion. No async `statusUrl`. | Persists `ImplementationStepLedgerRecorded`, `ImplementationStepBlocked`, or `ImplementationStepCompleted`; no file/shell/browser/deploy/external mutation is performed. Refetch the ledger after each record call. Founder Brief consumes the ledger progress report plus blocked/failed test evidence for implementation progress, Known Risks, and Next Validation Actions. | `RESOURCE_NOT_FOUND` for missing session, `STATE_VERSION_CONFLICT` for stale expected state, `VALIDATION_FAILED` for malformed body/unsupported keys/invalid evidence records/same review id/mismatched review range/test commit. Failed tests, missing test evidence, dirty no-code evidence, Not-tested gaps, credential/session/token/secret-shaped payload values, or credential-bearing URLs must remain visible blockers or validation failures. New dependency/external service/production mutation/credential needs must be split into approval or follow-up issues, not hidden in a completed step. |
| `GET /api/v1/sessions/:sessionId/implementation-step-ledger` | Auth required. Param: `sessionId`. No body. | `commandType: none`; latest implementation step ledger projection query. | `ApiSuccessEnvelope<ImplementationStepLedgerProjection | null>`; existing session with no implementation ledger returns `data: null`. | No effect. Refetch after `RecordImplementationStepLedger`. | `RESOURCE_NOT_FOUND` if session missing. |
...
- Implementation step ledger family
  - Purpose: make each Codex-built implementation step auditable through a linear ledger instead of only relying on the final PR commit.
  - Current #104 mounted surface: `POST /api/v1/sessions/:sessionId/implementation-step-ledger` and `GET /api/v1/sessions/:sessionId/implementation-step-ledger`.
