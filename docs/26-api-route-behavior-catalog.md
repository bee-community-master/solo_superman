...
| `POST /api/v1/sessions/:sessionId/chatgpt-browser-delegations` | Auth required. Param/body `sessionId` must match. Body DTO: `CreateChatGptBrowserDelegationRunRequest` with `idempotencyKey`, `expectedStateVersion`, `researchTaskId`, prompt preview ref, data disclosure preview, redaction summary, policy/session verdicts, approval decision, optional `browserActionAuthorityRef`, optional result import gate, fallback, screenshot/log/audit refs. | `commandType: CreateChatGptBrowserDelegationRun`; deterministic ProductEngine preflight/run record for the ChatGPT Pro local browser delegation contract. | `accepted_with_projection` with `ChatGptBrowserDelegationProjection`; ready runs return `currentStatus=ready_for_browser_action` or `result_import_ready`, blocked runs return `fallback_required` with visible fallback. No async `statusUrl`. | Persists `ChatGptBrowserDelegationRunRecorded` or `ChatGptBrowserDelegationRunBlocked` event/projection only. Browser capture still runs through the Phase 3 `browser_action` adapter and its `ExecutionAuthorityRecord`; this route does not store credentials/session values or silently retry. | `RESOURCE_NOT_FOUND` for missing session/research task, `STATE_VERSION_CONFLICT` for stale expected state, `VALIDATION_FAILED` for malformed body/unsupported keys/secret-shaped values. Missing data disclosure/redaction preview, missing user approval, policy/session custody, account sharing/resale, unattended queue, missing browser authority, or failed result-import quality gate creates blocked/fallback projection evidence. |
...
- ChatGPT browser delegation family
  ...
  - Current #101 mounted surface: `POST /api/v1/sessions/:sessionId/chatgpt-browser-delegations` and `GET /api/v1/sessions/:sessionId/chatgpt-browser-delegations`.
