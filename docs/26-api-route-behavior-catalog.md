...
- Application actions that do not issue a `ProductEngineCommand` must be explicitly marked `commandType: none`.
...
| `POST /api/v1/research-tasks/:researchTaskId/results` | Auth required. Param: `researchTaskId`. Body DTO: `ImportResearchResultRequest`. | `commandType: ImportResearchResult`. Payload includes pasted/manual result, source metadata, and limitation notes. | `accepted` or `accepted_with_projection`; `statusUrl` required when synthesis/queue update effects are queued. | May queue `research_evidence_effect` and `queue_projection_effect`. Emits research and queue projection updates. | `RESOURCE_NOT_FOUND`, `VALIDATION_FAILED` for missing source/result, `IDEMPOTENCY_CONFLICT` by task id + result hash. |
| `POST /api/v1/research-results/:researchResultId/synthesize` | Auth required. Param: `researchResultId`. Body DTO: `SynthesizeEvidenceRequest`. | `commandType: SynthesizeEvidence`. Payload from result ref and synthesis target. | `accepted` with `statusUrl` when async; `accepted_with_projection` only if synthesis is deterministic/immediate. | Queues `research_evidence_effect`; may queue `queue_projection_effect`. Refetch research, queue, confidence. | `COMMAND_PRECONDITION_FAILED` if result is empty, stale, or already terminal without force option. |
