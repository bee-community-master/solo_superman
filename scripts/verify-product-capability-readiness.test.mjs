...
function researchEvidenceCheckedBehaviors() {
  return [
    ...
    "Research-generated customer follow-up questions must use evidence-derived or domain-specific candidate lists; when research has not separated enough comparable customer candidates, they ask for direct idea-fit first-customer input instead of inventing generic founder/operator choices."
  ];
}
...
describe("product capability readiness verification", () => {
  ...
  it("requires each capability to name its exact verifier commands", () => {
    ...
    expect(result.issues).toEqual(expect.arrayContaining([
      ...
      "$.capabilities[4].verificationCommands: must include pnpm verify:single-session-product-loop:worker",
      "$.capabilities[4].verificationCommands: must include pnpm verify:single-session-product-loop:live-web",
      "$.capabilities[4].verificationCommands: must include pnpm verify:single-session-product-loop:live-implementation",
  ...
  it("requires the default verification command list to include product readiness", () => {
    ...
    expect(result.issues).toEqual(expect.arrayContaining([
      ...
      "$.requiredVerificationCommands.defaultSuite: must include pnpm verify:single-session-product-loop:worker",
  ...
  it("requires supporting commands to include live runtime, safe support, and ready-release plan checks", () => {
    ...
    expect(result.issues).toEqual(expect.arrayContaining([
      ...
      "$.requiredVerificationCommands.supporting: must include pnpm verify:single-session-product-loop:live-implementation",
