...
function replaceAutoImplementationTrackerRunState(existing: string | null, run: AutoImplementationRun) {
  const nextRunState = autoImplementationRunStateMarkdown(run);

  if (!existing) {
    return `${nextRunState}\n`;
  }

  const startIndex = existing.indexOf(AUTO_IMPLEMENTATION_TRACKER_RUN_STATE_START);
  const endIndex = existing.indexOf(AUTO_IMPLEMENTATION_TRACKER_RUN_STATE_END);

  if (startIndex >= 0 && endIndex > startIndex) {
    const afterEndIndex = endIndex + AUTO_IMPLEMENTATION_TRACKER_RUN_STATE_END.length;
    return `${existing.slice(0, startIndex).trimEnd()}\n\n${nextRunState}\n\n${existing.slice(afterEndIndex).trimStart()}`;
  }
