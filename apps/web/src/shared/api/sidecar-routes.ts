...
export function executionAuthorityPath(sessionId: SessionId) {
  return `/api/v1/sessions/${encodeURIComponent(sessionId)}/execution-authority`;
}
...
export function autoImplementationPullRequestMutationPath(sessionId: SessionId, runId: string) {
  return `${autoImplementationRunPath(sessionId)}/${encodeURIComponent(runId)}/pr-mutations`;
}
